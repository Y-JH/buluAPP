...
import com.bulu.cn.event.IAndroidFourthPageEvent;
 ...
 */
public class IAndroidCallbackFourthAgent {
    private Handler deliver = new Handler(Looper.getMainLooper());
    private AgentWeb mAgent;
    private Activity mActivity;

    public IAndroidCallbackFourthAgent(AgentWeb agent, Activity activity) {
        this.mAgent = agent;
     ...
     */
    @JavascriptInterface
    public void showAndroidNewFourthPage(final String pageUrl) {
...
//                Toast.makeText(BaseApplication.getsInstance(), "pageUrl=" + pageUrl, Toast.LENGTH_LONG).show();
                EventBusActivityScope.getDefault(mActivity)
                        .post(new IAndroidFourthPageEvent.NewPage(pageUrl));
            }
        });
    }
     ...
     */
    @JavascriptInterface
    public void closeAndroidFourthPage() {
        EventBusActivityScope.getDefault(mActivity)
                .post(new IAndroidFourthPageEvent.ClosePage());
    }

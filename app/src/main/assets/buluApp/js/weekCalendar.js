...
var currentDate = nowDate //当前日期

$(function() {

	$.ajax({
		type: "post",
		url: activeUrl + "/xxtd/xxtdKsList",
		data: {
			"xyid": xyid
		},
		async: false,
		...
		success: function(data) {
			...
			for(var i = 0; i < data.data.kclist.length; i++) {

				var std = ''
				std += `<div class="slide">
							<a class="mui-control-item static-color kcdg" href="#item1mobile" id=${data.data.kclist[i].id}>
							${data.data.kclist[i].course_name}
						</a>
						</div>`
				$('.mui_xxk').append(std)
				mathClass = data.data.kclist[0].id

			}
			//			alert(mathClass)
			//			alert(currentDate)
			$(document).on('click', '.kcdg', function() {
				mathClass = $(this).attr('id')
				//				alert('日期' + currentDate)
				//				alert('课程' + mathClass)
				$.ajax({
					url: activeUrl + "/xxtd/xxtdksByKcid",
					type: "post",
					data: {
						"xyid": xyid,
						"kcid": mathClass,
					...
					success: function(data) {
						...
						for(var i = 0; i < data.data.length; i++) {
							var newdater = (data.data[i].date).substr(0, 16)
							var newdated = (data.data[i].time_bucket).substr(11, 5)
							var sds = ''
							sds += `<div class="main_font">
							        <h4 class="hdr">${data.data[i].course_name}</h4>
							        <p>班级: ${data.data[i].class_name}</p>
							        <p>讲课老师: ${data.data[i].teachername}</p>
							        <p>教室: 教学楼${data.data[i].classroom}</p>
							        <p class="padd">${newdater}-${newdated}</p>
							        <div class="btn">`
							if(data.data[i].sfyy > 0) {
								sds += `
						<button type="button" class="mui-btn mui-btn-outlined" style="background: #ccc; color: white;"  data-id="${data.data[i].pkid}">已预约</button>
					    `
							} else {
								sds += `
						<button type="button" class="mui-btn mui-btn-outlined yuyues" style="background-color: rgb(255,139,54); color: white;" data-id="${data.data[i].pkid}">预约</button>
					     `
							}
							sds += `
			...
			for(var i = 0; i < data.data.kslist.length; i++) {
				var newdatet = (data.data.kslist[i].date).substr(0, 16)
				var newdatee = (data.data.kslist[i].time_bucket).substr(11, 5)
				var sts = ''
				sts += `<div class="main_font">
							        <h4 class="hdr">${data.data.kslist[i].course_name}</h4>
							        <p>班级: ${data.data.kslist[i].class_name}</p>
							        <p>讲课老师: ${data.data.kslist[i].teachername}</p>
							        <p>教室: 教学楼${data.data.kslist[i].classroom}</p>
							        <p class="padd">${newdatet}-${newdatee}</p>
							        <div class="btn">`
				if(data.data.kslist[i].sfyy > 0) {
					sts += `
						<button type="button" class="mui-btn mui-btn-outlined" style="background: #ccc; color: white;"  data-id="${data.data.kslist[i].pkid}">已预约</button>
					    `
				} else {
					sts += `
						<button type="button" class="mui-btn mui-btn-outlined yuyues" style="background-color: rgb(255,139,54); color: white;" data-id="${data.data.kslist[i].pkid}">预约</button>
					     `
				}
				sts += `
	...
	$(document).on("click", ".yuyues", function() {
		$(this).text('已预约')
		$(this).addClass('aa')
		var ksid = $(this).attr("data-id")
		//		console.log(ksid)
		//		console.log(xyid)
		$.ajax({
			url: activeUrl + "/xxtd/xxtdYyks",
			type: "post",
			data: {
				"xyid": xyid,
				"pkid": ksid
	...
	var weekCalendar_plugin = {
		...
		bindEvent: function() {
			...
			$(".wcDate>li").click(function(e) {
				...
				//          	点击日期获取课程数剧
				currentDate = year + "-" + month + "-" + currentDate;
				//				alert('日期' + currentDate)
				//				alert('课程' + mathClass)
				$.ajax({
					url: activeUrl + "/xxtd/xxtdksByKcid",
					...
					success: function(data) {
						...
						for(var i = 0; i < data.data.length; i++) {
							...
							var newdatef = (data.data[i].time_bucket).substr(11, 5)
							var srs = ''
							srs += `<div class="main_font">
							        <h4 class="hdr">${data.data[i].course_name}</h4>
							        <p>班级: ${data.data[i].class_name}</p>
							        <p>讲课老师: ${data.data[i].teachername}</p>
							        <p>教室: 教学楼${data.data[i].classroom}</p>
							        <p class="padd">${newdateu}-${newdatef}</p>
							        <div class="btn">`
							if(data.data[i].sfyy > 0) {
								srs += `
						<button type="button" class="mui-btn mui-btn-outlined" style="background: #ccc; color: white;"  data-id="${data.data[i].pkid}">已预约</button>
